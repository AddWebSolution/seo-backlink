...
export function useKeywordReportApi() {
  ...
  const fetchKeywordReport = async (id, body = {}) => {
    loading.value = true;
    error.value = null;
    try {
      const result = await useApi(`api/keywordreport/keyword/${id}`, {
        method: "POST",
        body,
      });
  ...
  // get all reports
  const fetchKeywordReports = async (filters = {}, page = null) => {
    loading.value = true;
    error.value = null;
    try {
      const body = {
      ...
      const result = await useApi(
        createUrl("api/keywordreport/get", { query: body }),
        {
          method: "POST",
        }
      );
  ...
  // delete report
  const deleteKeywordReport = async (id) => {
    loading.value = true;
    error.value = null;

    try {
      const result = await useApi(`api/keywordreport/delete/${id}`, {
        method: "POST",
      });
  ...
  // export reports
  const exportKeywordReports = async (requestBody) => {
    loading.value = true;
    error.value = null;

    try {
      const response = await fetch("/api/keywordreport/export", {
        method: "POST",
        headers: {
          Accept:
